...
#Script per creare le curve in formato shp della regione intera (20 metri)
...
for filename in $(ls *.txt); do
	...
	cp ../$tif/DTM5_$codice.tif ../$curve20re/$n/
	done
	#crea il file vrt unico per la provincia
	echo
	echo "Creo il file vrt per la provincia di $n"
	gdalbuildvrt ../$curve20re/$n/SHP20_$n.vrt ../$curve20re/$n/*.tif -a_srs "EPSG:32632"
	#crea le curve
	echo
	echo "Creo le curve della provincia di $n"
	gdal_contour -b 1 -a name -i 20.0 -f "ESRI Shapefile" ../$curve20re/$n/SHP20_$n.vrt "../$curve20re/$n/SHP20_$n.shp"
	#ritaglia le curve sul confine della provincia
	echo
	echo "Taglio le curve che sbordano oltre il confine della provincia di $n"
	ogr2ogr -f 'ESRI Shapefile' -update -append -progress -clipsrc ../Taglio/Shp/$n.shp ../$curve20re/SHP20_Piemonte.shp ../$curve20re/$n/SHP20_$n.shp
...
cp -r ./Documentazione/Documentazione_SHP20_Piemonte ./$curve20re/Documentazione


#si sposta nella cartella documentazione
cd ./$curve20re/Documentazione
