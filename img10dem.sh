...
rm -r $uscitaimg10/GPS_dem10m
rm -r $uscitaimg10/NewGPS_dem10m
rm -r $uscitaimg10/Basemap_dem10m
rm $uscitaimg10/*_dem10m-*.zip
...
for infile in $uscitaimg10/66100*.osm.pbf
  do
  ...
  java $Xmx -jar $mkgmap --code-page=1252 \
    ...
    --dem-poly=Taglio/Poly/Piemonte.poly \
  $infile
done
...
# compilo il file typ dal formato testo
     java -jar $mkgmap \
     --family-id=1811 \
     Stile_Garmin/Typ/${TYPFILEdem_VERSION}.txt

mv ${TYPFILEdem_VERSION}.typ Stile_Garmin/Typ

MASTER_TYPFILE=Stile_Garmin/Typ/${TYPFILEdem_VERSION}.typ
...
DTM5_Curve=$(ls $uscitaimg10/66100*.img)

# make the target directory
mkdir $uscitaimg10/GPS_dem10m
mkdir $uscitaimg10/NewGPS_dem10m
mkdir $uscitaimg10/Basemap_dem10m

# unisco il file di ogni strato in un file IMG separato per i nuovi disposistivi come ad es. il gps64

$GMT -j -o $uscitaimg10/NewGPS_dem10m/Curve_dem10m.img \
     -f 1811,1 -m "DTM5 Curve_dem10m" $DTM5_Curve ./Stile_Garmin/Typ/1811.TYP

    
# gli strati ora vengono uniti in un unico gmapsupp.img per i vecchi dispositivi:

$GMT -j -o $uscitaimg10/GPS_dem10m/gmapsupp.img -m "DTM5 Curve_dem10m (GPS)" \
     $uscitaimg10/NewGPS_dem10m/Curve_dem10m.img
...
java -jar $mkgmap \
  --series-name="DTM5 Curve_dem10m (PC version)" \
  --overview-mapname="mapset"   \
  --country-name="Italia"       \
  --region-name="Piemonte"      \
  --output-dir=$uscitaimg10/Basemap_dem10m     \
  --family-id=1811              \
  --draw-priority=30            \
...
rm $uscitaimg10/Basemap_dem10m/mapset.tdb


# Facio un gmapsupp.img intermedio, lo utilizziamo per poi dividerlo
# nella creazione dei file per Basecamp:

$GMT -j -o $uscitaimg10/Basemap_dem10m/gmapsupp.img \
     -m "DTM5 Curve_dem10m (PC version)" \
     -f 1811,1		\
     $DTM5_Curve        \
     $MASTER_TYPFILE
...
$GMT -S \
     -f 1811,1 \
     -o $uscitaimg10/Basemap_dem10m \
     $uscitaimg10/Basemap_dem10m/gmapsupp.img

# cancello il file intermedio gmapsupp.img

rm $uscitaimg10/Basemap_dem10m/gmapsupp.img
...
python Stile_Garmin/tdbfile.py $uscitaimg10/Basemap_dem10m/mapset.tdb
...
cp -r Documentazione/Garmin_dem/Documentazione $uscitaimg10/Basemap_dem10m

cp -r Documentazione/GPS_dem/Documentazione $uscitaimg10/GPS_dem10m

cp -r Documentazione/NewGPS_dem/Documentazione $uscitaimg10/NewGPS_dem10m
...
zip -r "Basemap_dem10m-$data.zip" Basemap_dem10m

echo
echo
echo "Creo il file zip per GPS"
zip -r "GPS_dem10m-$data.zip" GPS_dem10m

echo
echo
echo "Creo il file zip per GPS separato"
zip -r "NewGPS_dem10m-$data.zip" NewGPS_dem10m
