...
#Script per la creazione delle curve 10 metri in formato OSM
...
if [ -d $uscitaosm10 ]; then
    echo "OK - $uscitaosm10 esiste."
else
    mkdir $uscitaosm10
fi
...
#verifica se è presente il file con le curve a 10 metri della Regione, altrimenti li crea
if [ -f $curve10re/SHP10_Piemonte.shp ]; then
    echo "OK - $curve10re/SHP10_Piemonte.shp esiste."
    echo 
    echo "Copio il file nella cartella"
    cp ./$curve10re/SHP10*.* ./$uscitaosm10/
else
	...
	for filename in $(ls Regione.txt); do
		...
		cp ../$tif/DTM5_$codice.tif ../$uscitaosm10/$n/
		done
		#crea il file vrt unico per la regione
		echo
		echo "Creo il file vrt per la Regione"
		gdalbuildvrt ../$uscitaosm10/$n/SHP10_$n.vrt ../$uscitaosm10/$n/*.tif -a_srs "EPSG:32632"
		#crea le curve
		echo
		echo "Creo le curve della Regione"
		gdal_contour -b 1 -a name -i 10.0 -f "ESRI Shapefile" ../$uscitaosm10/$n/SHP10_$n.vrt "../$uscitaosm10/SHP10_Piemonte.shp"
...
for filename in $(ls *.shp); do
	...
	ogr2ogr -f "ESRI Shapefile" -explodecollections -clipsrc $filename ../../../$uscitaosm10/$s.shp ../../../$uscitaosm10/SHP10_Piemonte.shp
...
echo
echo Unisco i file
osmium merge *.pbf -o curve10.pbf
